...
y_test = wc_b_result
'''
plr_X = np.concatenate((normal_b_team_records, normal_r_team_records), axis=1) #player one hot
b_result = np.array(normal_data.loc[normal_data['playerid'].isin([100])]['result'])
plr_y = b_result

X_test = np.concatenate((wc_b_team_records, wc_r_team_records), axis=1) #player one hot
wc_b_result = np.array(wc_data.loc[wc_data['playerid'].isin([100])]['result'])
y_test = wc_b_result
'''
