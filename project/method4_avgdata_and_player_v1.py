...
def makeNormalDataAvgRecords(normal_data, important_records):
    ...
    for i in range(normal_team_data.shape[0]):
        team_name = normal_team_data.iloc[i]['team']
        team_avg_records_df.loc[team_name] += normal_team_data[important_records].iloc[i]/team_records_count.loc[team_name]['times']
...
#wc_data = wc_data[~wc_data['week'].isin(['1.1', '1.2', '1.3', '1.4', '1.5', '1.6'])]

#encode records to train
...
normal_b_team_records = normal_data[normal_data['playerid'].isin([100])][important_records]
normal_r_team_records= normal_data[normal_data['playerid'].isin([200])][important_records]
...
wc_plr_b_one_hot = np.zeros((int(wc_data.shape[0]/12),len(oe.categories_[0]))) #[40,449]
